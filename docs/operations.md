...
| Supabase database | one failed probe | unavailable for 2 minutes |
...
Use Supabase managed backups and point-in-time recovery in production. Export a logical backup with `supabase db dump -f ai-trading.sql` when an additional portable snapshot is required.
...
Restore into an isolated Supabase project, never over the active project.

Then:

1. Run `supabase migration list` against the restored project.
...
3. Restore Supabase connectivity.
